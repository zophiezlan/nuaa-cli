...
# Agent manifest path and helpers
...
# Return a pipe-separated list of supported agent keys from the manifest (or fallback)
agent_keys_pipe() {
    if [ "$JQ_PRESENT" -eq 1 ]; then
        jq -r 'keys | join("|")' "$AGENTS_JSON_PATH"
        return
    fi
    echo "claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|roo|codebuddy|amp|q"
}
...
print_summary() {
    ...
    log_info "Usage: $0 [$(agent_keys_pipe)]"
}
