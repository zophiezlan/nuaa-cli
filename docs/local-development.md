...
uvx --from git+https://github.com/zophiezlan/nuaa-cli.git@your-feature-branch nuaa init demo-branch-test --script ps
```
...
```bash
uvx --from /path/to/nuaa-cli nuaa --help
uvx --from /path/to/nuaa-cli nuaa init demo-anywhere --ai copilot --ignore-agent-tools --script sh
```

Set an environment variable for convenience:

```bash
export NUAA_SRC=/path/to/nuaa-cli
...
```bash
nuaa-dev() { uvx --from /path/to/nuaa-cli nuaa "$@"; }
# Then
nuaa-dev --help
```
...
| Local uvx run (abs path)  | `uvx --from /path/to/nuaa-cli nuaa ...`                |
